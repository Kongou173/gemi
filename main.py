...
from history import HistoryHandler, to_jsonable
...
async def on_message(message: discord.Message):
    ...
    async with message.channel.typing():
        with history_handler(get_current_history(message.author.id)) as history:
            ...
            res, errors = await gemini.generate(use_history + [content], KEYS)
            if res is not None:
                ...
                if len(res_text) > 2000:
                    await message.reply(file=discord.File(io.BytesIO(res_text.encode()), 'text.txt'))
                else:
                    await message.reply(res_text, suppress=True)
                history.append(content)
                history.append(res_content)
            else:
                await message.reply(f'{str(errors)} {len(errors)}')

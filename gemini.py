...
GEMINI_MODEL = config_ini['MAIN']['GeminiModel']
SAFETY_SETTINGS = [
    {
...
class Gemini:
    ...
    async def generate(self, contents: list[dict], keys: list[str], proxy: str | None = None):
        data = {
            'contents': contents,
        ...
        for key in keys:
            ...
            async with self.session.post(
                URL.format(model=GEMINI_MODEL), params=params, json=data, proxy=proxy
            ) as response:
